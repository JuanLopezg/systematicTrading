...
for id_value, id_df in df.groupby('id', sort=False):
    ...
    valid_times = id_df[id_df['date'].dt.is_month_start]
    ...
    # Iterate from one month-start to the next
    for i in range(len(valid_times) - 1):
        start_time = valid_times.iloc[i]['ts']
        end_time = valid_times.iloc[i + 1]['ts']

        # Select the data between month i and i+1
        slice_df = id_df[(id_df['ts'] >= start_time) & (id_df['ts'] < end_time)]
