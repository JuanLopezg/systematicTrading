...
if __name__ == "__main__":
    ...
    df_hl = df_top50_full[df_top50_full["symbol"].apply(lambda sym: matches(sym, hl_perps))].reset_index(drop=True)

    # Ajouter une colonne "perp" avec les noms des perps HL correspondants
    df_hl["perp"] = df_hl["symbol"].apply(lambda sym: find_matching_perp(sym, hl_perps))
    ...
    ohlcv_data = df_id_perp["perp"].apply(lambda p: hl_api.fetchDailyHyperliquid(p, nDays=1, offset=1).iloc[0])
